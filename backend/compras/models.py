...
class OrdenCompra(models.Model):
    ...
    def __str__(self):
        return f"OC #{self.id_orden_compra} - Proveedor: {self.id_proveedor}"

class OrdenCompraMateriaPrima(models.Model):
    id_orden_compra_materia_prima = models.AutoField(primary_key=True)
    id_orden_compra = models.ForeignKey(OrdenCompra, on_delete=models.CASCADE, db_column="id_orden_compra", related_name="materias_primas")
    ...
    def __str__(self):
        return f"{self.id_materia_prima} ({self.cantidad}) - OC #{self.id_orden_compra.id_orden_compra}"
...
class OrdenCompraProduccion(models.Model):
    ...
    def __str__(self):
        return f"OC #{self.id_orden_compra.id_orden_compra} vinculada a OP #{self.id_orden_produccion.id_orden_produccion}"