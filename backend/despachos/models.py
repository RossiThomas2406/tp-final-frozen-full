...
class OrdenDespacho(models.Model):
    ...
    def __str__(self):
        f_despacho = self.fecha_despacho.strftime('%d/%m/%Y %H:%M') if self.fecha_despacho else "Sin fecha"
        return f"Orden #{self.id_orden_despacho} - {f_despacho}"
...
class DespachoOrenVenta(models.Model):
    ...
    def __str__(self):
        return f"Despacho Venta #{self.id_orden_venta} (Orden Despacho #{self.id_orden_despacho})"