...
class Receta(models.Model):
    ...
    def __str__(self):
        return f"Receta: {self.id_producto.nombre}"


class RecetaMateriaPrima(models.Model):
    id_receta_materia_prima = models.AutoField(primary_key=True)
    id_receta = models.ForeignKey(Receta, on_delete=models.CASCADE, db_column="id_receta", related_name="ingredientes")
    ...
    def __str__(self):
        return f"{self.id_materia_prima.nombre} para {self.id_receta.id_producto.nombre}"
...
class ProductoLinea(models.Model):
    ...
    def __str__(self):
        return f"{self.id_producto.nombre} en {self.id_linea_produccion.descripcion}"