...
def crear_orden_venta(request):
    if request.method == "POST":
        try:
            ...
            # Buscamos el estado inicial de forma robusta
            try:
                estado_creada = EstadoVenta.objects.get(descripcion__iexact="Creada")
            except EstadoVenta.DoesNotExist:
                # Si no existe 'Creada', usamos 'Pendiente' como fallback
                estado_creada = EstadoVenta.objects.get(descripcion__iexact="Pendiente")
            ...
            with transaction.atomic():
                ...
                # 1. Crear la orden de venta (Sin comas al final de los campos)
                ...
                # 2. Crear los productos vinculados
                ...
                # 3. LÓGICA DE DECISIÓN (Online vs Empresarial)
                if tipo_venta_enviado == 'ONL':
                    resultado_online = procesar_orden_venta_online(orden_venta)
                    if not resultado_online['exito']:
                        raise Exception(f"No se pudo procesar la venta online: {resultado_online['mensaje']}")
                else:
                    # Venta Empresarial (EMP): Registra y actualiza estado para planificador
            ...
            # Django hace ROLLBACK automático si ocurre una excepción dentro de 'atomic'
            return JsonResponse({"error": str(e)}, status=400)
