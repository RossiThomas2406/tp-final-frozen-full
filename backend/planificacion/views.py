...
def ejecutar_planificador_view(request):
    ...
    try:
        ...
        return Response(
            {
                "status": "ok",
                "message": f"Planificador MRP ejecutado para {fecha_a_usar}.",
            },
            status=status.HTTP_200_OK,
        )
    except Exception as e:
        ...
        return Response(
            {
                "status": "error",
                "message": f"Error al ejecutar el planificador: {str(e)}",
            },
            status=status.HTTP_500_INTERNAL_SERVER_ERROR,
        )
...
def replanificar_capacidad_view(request):
    ...
    try:
        replanificar_ops_por_capacidad(
            fecha_simulada=fecha_a_usar, productos_a_replanificar_ids=productos_ids
        )
        ...
        return Response({"status": "ok", "message": mensaje}, status=status.HTTP_200_OK)
        ...
        return Response(
            {
                "status": "error",
                "message": f"Error en la replanificación de capacidad: {str(e)}",
            },
            status=status.HTTP_500_INTERNAL_SERVER_ERROR,
        )
        ...
        try:
            ...
            for ov in ovs_pendientes:
                ...
                try:
                    ...
                    total_productos = (
                        OrdenVentaProducto.objects.filter(id_orden_venta=ov).aggregate(
                            total=Sum("cantidad")
                        )["total"]
                        or 0
                    )
